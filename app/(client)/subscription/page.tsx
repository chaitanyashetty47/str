...
// Define subscription plan types (UI-friendly values)
...
// Map database enum values to UI-friendly category values
const mapCategory = (dbCategory: string): PlanCategory => {
  if (dbCategory === "FITNESS" || dbCategory === "fitness") return "fitness"
  if (dbCategory === "PSYCHOLOGY" || dbCategory === "psychology") return "psychological"
  return "manifestation"
}

// Map database enum values to UI-friendly plan type values
const mapPlanType = (dbPlanType: string | null): PlanType | null => {
  if (!dbPlanType) return null
  switch (dbPlanType) {
    case "ONLINE":
      return "online"
    case "IN_PERSON":
    ...
    async function fetchPlans() {
      ...
      const transformedPlans = data.map((plan: any) => {
        ...
        return {
          ...
          plan_type: mapPlanType(plan.planType),
          price: plan.price,
          features: features,
