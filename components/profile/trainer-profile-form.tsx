...
export function TrainerProfileForm({ user, initialData, onDataUpdate }: TrainerProfileFormProps) {
  ...
  const { execute: executeUpdate, isLoading, fieldErrors, error } = useAction(updateTrainerProfile, {
    onSuccess: (data) => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const profileData = await getTrainerProfileDetails();
        setProfile(profileData);
        setFormDataFromProfile(profileData);
      } catch (error) {
        console.error("Error fetching trainer profile:", error);
        toast.error("Failed to load trainer profile data");
      } finally {
        setIsLoadingProfile(false);
      }
    };
  ...
  if (isLoadingProfile) {
    return <div className="flex justify-center p-8">Loading trainer profile...</div>;
  }

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium">Trainer Profile Information</h3>
        {!isEditing ? (
          <Button 
            variant="outline"
