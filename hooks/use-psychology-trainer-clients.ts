...
import { fetchPsychologyTrainerClients } from "@/actions/trainer-clients/psychology/psychology-trainer-clients.action";
import useSWR from "swr";
import { TrainerClientsFilters, TrainerClientsResponse, TrainerClientRow } from "@/types/trainer-clients.types";

export const usePsychologyTrainerClients = (initialData?: TrainerClientsResponse): {
  // Data
  ...
  const fetcher = async (key: string) => {
    ...
    const result = await fetchPsychologyTrainerClients(params);
    if (result.error) {
  ...
  const { data: swrData, error, isLoading: swrLoading, mutate } = useSWR(
    ...
    fetcher,
    {
      ...
      errorRetryCount: 3,
      onError: (error) => {
        console.error("Failed to fetch psychology trainer clients:", error);
      },
    }
  );
  ...
  const fetchClients = useCallback(async () => {
    console.log('Manually refetching psychology trainer clients data...');
