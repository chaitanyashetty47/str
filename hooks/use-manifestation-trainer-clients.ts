...
import { fetchManifestationTrainerClients } from "@/actions/trainer-clients/manifestation/manifestation-trainer-clients.action";
import useSWR from "swr";
import { TrainerClientsFilters, TrainerClientsResponse, TrainerClientRow } from "@/types/trainer-clients.types";

export const useManifestationTrainerClients = (initialData?: TrainerClientsResponse): {
  // Data
  ...
  const fetcher = async (key: string) => {
    ...
    const result = await fetchManifestationTrainerClients(params);
    if (result.error) {
  ...
  const { data: swrData, error, isLoading: swrLoading, mutate } = useSWR(
    ...
    fetcher,
    {
      ...
      errorRetryCount: 3,
      onError: (error) => {
        console.error("Failed to fetch manifestation trainer clients:", error);
      },
    }
  );
  ...
  const fetchClients = useCallback(async () => {
    console.log('Manually refetching manifestation trainer clients data...');
