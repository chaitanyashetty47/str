...
export async function getPsychologyTrainerClients(
  trainerId: string,
  query: TrainerClientsQuery
): Promise<TrainerClientRow[]> {
  const { page, pageSize, search, dateRange, sort } = query;

  // Subscription plans categories we want to include (PSYCHOLOGY trainer only)
  const ALLOWED_CATEGORIES: SubscriptionCategory[] = [
    SubscriptionCategory.PSYCHOLOGY,
  ...
  const whereClause = {
    ...
    // clients assigned to this trainer for PSYCHOLOGY category
...
export async function getPsychologyTrainerClientsCount(
  trainerId: string,
